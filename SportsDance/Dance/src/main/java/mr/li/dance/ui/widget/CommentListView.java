...
public class CommentListView extends LinearLayout {
    ...
    private DetailsInfo.CommBean    mDatas;
    ...
    public void setDatas(DetailsInfo.CommBean datas) {
        if (datas == null) {
            datas = new DetailsInfo.CommBean();
        }
        mDatas = datas;
        notifyDataSetChanged();
    }

    public DetailsInfo.CommBean getDatas() {
        return mDatas;
    }
    ...
    public void notifyDataSetChanged() {
        ...
        removeAllViews();
        if (mDatas == null || mDatas.getComd().size() == 0) {
            return;
        }
        LayoutParams layoutParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        for (int i = 0; i < mDatas.getComd().size(); i++) {
            final int index = i;
    ...
    private View getView(final int position) {
        ...
        String name = mDatas.getUser().get(0).getUsername();
        String id = mDatas.getId();
        String toReplyName = "";
        if (mDatas != null) {
            toReplyName = mDatas.getComd().get(position).getUser().get(0).getUsername();
        }

        SpannableStringBuilder builder = new SpannableStringBuilder();

        builder.append(setClickableSpan(toReplyName, mDatas.getComd().get(position).getId()));
        ...
        builder.append(": ");
        //转换表情字符
        String contentBodyStr = mDatas.getComd().get(position).getContent();
